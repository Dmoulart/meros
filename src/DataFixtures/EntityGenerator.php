...
class EntityGenerator
{
     ...
     */
    function generate(string $class, int $n = 10): array | User | Vehicle
    {
        if($n === 1) return $this->generateOne($class);
        else return $this->generateMany($class,$n);
    }
     ...
     * @return User|Vehicle
     */
    private function generateOne(string $class): User | Vehicle
    {
        return match ($class) {
    ...
    private function generateMany(string $class, int $n = 10): array
    {
        ...
        $entities = array_fill(0, $n,null);

        return array_map(
            function() use ($genFunc)
            {
                return $this->$genFunc();
            },
            $entities
